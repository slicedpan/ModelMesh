...
void main()
{
	...
	vec3 lightAmount = clamp(dot(lightVec, pixNormal), 0.1, 1.0) * vec3(1);	
	vec3 diffuseColor = lightAmount * texture2D(diffuseMap, vec2(texCoord.x, texCoord.y * -1)).xyz * LightColor;
